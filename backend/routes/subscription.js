...
router.post('/restore-purchases', protect, async (req, res) => {
  try {
    ...
    // Validate receipt before restoring
    let validationResult;
    if (platform === 'ios') {
      validationResult = await validateAppleReceipt(receipt);
    } else {
      validationResult = await validateGoogleReceipt(receipt, 'afroconnect_premium_monthly');
    }

    if (!validationResult.valid) {
      return res.status(402).json({ success: false, message: 'Could not verify purchase to restore.' });
    }
