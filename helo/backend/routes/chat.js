...
router.get('/conversations', protect, async (req, res) => {
  try {
    ...
    const lastMessagesAgg = await Message.aggregate([
      { $match: { matchId: { $in: matchIds }, deletedFor: { $ne: req.user._id } } },
      { $sort: { createdAt: -1 } },
      { $group: { _id: '$matchId', lastMessage: { $first: '$$ROOT' } } }
    ]);
    const lastMessagesMap = new Map(lastMessagesAgg.map(m => [m._id.toString(), m.lastMessage]));
    
    // Batch query: Get unread counts for all matches at once
    const unreadCountsAgg = await Message.aggregate([
      { $match: { matchId: { $in: matchIds }, receiver: req.user._id, seen: false, deletedFor: { $ne: req.user._id } } },
    ...
    const conversations = matches.map((match) => {
      ...
      const otherUserBlockedMe = (otherUser.blockedUsers || []).some(id => id.toString() === req.user._id.toString());
    ...
    const filteredConversations = conversations
      ...
      .filter(c => {
        if (seenUserIds.has(c.user.id.toString())) {
          return false;
        }
...
router.post('/:matchId', protect, async (req, res) => {
  try {
    ...
    // If matchId is 'direct', we expect receiverId in body
    if (matchId === 'direct') {
      if (!req.user.premium?.isActive) {
        return res.status(403).json({ success: false, message: 'Direct messaging without a match is a Premium feature. Upgrade now!' });
      }
      // logic to handle direct message would go here
    }
    ...
    if (currentUser?.blockedUsers?.includes(receiver.toString()) || 
    ...
    // Enforce Voice Note Limits
    if (type === 'audio') {
      const freeLimit = 30; // 30 seconds
      const isPremium = req.user.premium?.isActive;
      if (!isPremium && audioDuration > freeLimit) {
        return res.status(403).json({ success: false, message: 'Voice notes longer than 30s require Premium' });
      }
    }
...
router.put('/:matchId/read', protect, async (req, res) => {
  try {
    ...
    // Only premium users can see read receipts from others
    // We send the read status anyway, but frontend should respect the limit
    // Or we handle it here by NOT emitting if sender isn't premium
    const io = req.app.get('io');
    if (io) {
      const match = await Match.findById(matchId);
      const otherUserId = match.users.find(id => !id.equals(req.user._id));
      const otherUser = await User.findById(otherUserId).select('premium');
      
      if (otherUser?.premium?.isActive) {
        io.to(matchId.toString()).emit('chat:message-read', {
          chatId: matchId,
...
router.put('/message/:messageId/seen', protect, async (req, res) => {
  try {
    ...
    const io = req.app.get('io');
    if (io) {
      io.to(message.matchId.toString()).emit('chat:message-status', {
        messageId: message._id,
        status: 'seen'
      });
    }
...
router.delete('/message/:messageId', protect, async (req, res) => {
  try {
    ...
    // Delete for everyone (only sender can do this, within 15 minutes)
    if (deleteForEveryone === 'true') {
      if (!message.sender.equals(req.user._id)) {
        return res.status(403).json({ success: false, message: 'Only sender can delete for everyone' });
      }

      // Access message unsend: only for premium users
      const sender = await User.findById(message.sender).select('premium');
      if (!sender?.premium?.isActive) {
        return res.status(403).json({ success: false, message: 'Deleting for everyone is a Premium feature' });
      }
...
router.delete('/:matchId', protect, async (req, res) => {
  try {
    ...
    const isUserInMatch = match.users.some(userId => userId.equals(req.user._id));
    if (!isUserInMatch) {
      return res.status(403).json({ success: false, message: 'Not authorized' });
    }
