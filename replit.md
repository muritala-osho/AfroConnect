...
- **Gateway**: A gateway proxies API requests to the backend (port 3001) and serves the Expo web frontend (Webpack on port 19006) from an entry point (port 5000), enabling unified URL handling. Also serves admin dashboard static files at `/admin-web` and `/assets/` for static asset files (with path traversal protection). Expo is started with `--web` flag which launches Webpack dev server on port 19006 for web builds.
- **Web Keyboard Handling**: On mobile web, `KeyboardAvoidingView` is disabled (doesn't detect virtual keyboards). Instead, `window.visualViewport` resize events track keyboard height and apply `marginBottom` to the input container. Native iOS uses KAV with `behavior="padding"`, Android uses `Keyboard.addListener` for manual height tracking.
...
- **Communication**: Real-time chat with message deletion, translation, and reply features. Dynamic chat bubble styles (rounded/sharp/minimal) from `useTheme`. Agora Web SDK integration for robust voice and video calls. VoiceCallScreen has full controls: mute, speaker toggle (earpiece/loudspeaker via `Audio.setAudioModeAsync`), message shortcut, hangup. VideoCallScreen adds speaker toggle alongside camera, mute, flip, message, hangup. Both use hidden WebView for native Agora audio/video (agora-call.html), web uses agoraService directly. Ringtones: incoming=`mixkit-waiting-ringtone-1354.wav`, outgoing=`phone-calling-1b.mp3`. Offline call push notifications include full callData+callerName+callerPhoto for notification-tap join. Per-user mute settings (`muteSettings.mutedUsers`) respected for both message and call push notifications.
...
- **Premium Features**: In-app purchase integration for premium subscriptions (Google Play Billing / Apple IAP) with 12 features: Unlimited Likes, See Who Likes You, 10 Super Likes Daily, Unlimited Rewinds, Incognito Mode, Monthly Free Boost, Global Discovery with Country Picker, Unlimited Voice/Video Calls, Story Viewer Details, Advanced Filters, Premium Badge, No Distance Limits. Animated gold PremiumBadge component (`helo/components/PremiumBadge.tsx`) with dual-layer pulsing glow + shimmer color transition using react-native-reanimated, theme-aware glow colors (warmer amber in dark mode, classic gold in light mode), shown on MyProfileScreen, ProfileDetailScreen, and DiscoveryScreen cards. PremiumScreen shows "Coming Soon" on native until real IAP SDK is integrated; web shows "Mobile Only" message. Backend subscription routes: `/plans`, `/status`, `/validate-receipt`, `/restore-purchases`. Server-side interval validation whitelists day/week/month/year.
...
- **Admin Dashboard**: Two versions - vanilla HTML (`helo/admin-dashboard/index.html` with `css/style.css` and `js/dashboard.js`) served at `/admin-web`, and React-based (`helo/admin-dashboard/App.tsx` with views/). Both connect to backend API via `adminApi.ts`. JWT auth with isAdmin check. Vanilla HTML dashboard has a login overlay screen with AfroConnect logo that authenticates via `/api/auth/login` and checks `isAdmin` before showing dashboard. All data is real from backend: stats from `/api/admin/stats`, users from `/api/admin/users`, reports from `/api/admin/reports`, payments from `/api/admin/subscriptions-revenue`, support from `/api/support/tickets`. Charts use real data (user growth derived from totalUsers, revenue from planBreakdown, country distribution from user locations). Admin can update profile (name, bio, avatar) via profile modal using `PUT /api/users/me` and `POST /api/upload/photo`. Sidebar shows real admin name/avatar. Logout button clears session. Dashboard views: DashboardHome (stats, activity monitoring with real API + mock fallbacks), UserManagement (search, ban/unban), ReportsQueue (resolve reports), Payments (subscription revenue with mock fallback), IDVerification (approve/reject), Analytics (with Gemini AI), Support tickets. Backend admin routes in `helo/backend/routes/admin.js`.
...
- **In-App Purchases**: Google Play Billing (Android) and Apple IAP (iOS) for premium subscriptions. Backend has `validate-receipt` and `restore-purchases` endpoints ready for real store receipt validation (currently gated until store API keys are configured). Stripe code removed from active use but files retained in `stripe/` directory.
