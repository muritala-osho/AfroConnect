...
*   **Per-message read receipts**: Server emits `chat:message-read` (and legacy `chat:read`) on the match room with `{ chatId, matchId, userId, readAt }` whenever the recipient marks messages as read (HTTP `PUT /api/chat/:matchId/read`, single-message `PUT /api/chat/message/:id/seen`, or socket `chat:read`). The chat screen flips own messages to `status: 'seen'` AND stamps `seenAt: readAt` (preferring an existing `seenAt` if one is already on the message, so we never overwrite the original read time). The footer of the **last** own seen message renders a small italic `Seen at HH:MM` line — only on the latest seen message, matching WhatsApp/iMessage to avoid noisy footers. The `lastSeenOwnMessageId` is computed in a `useMemo` over `enrichedMessages` and added to `renderMessage`'s dep array.
